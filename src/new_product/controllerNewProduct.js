...
export const readNewProduct= async (request, response) => {
    try {
        ...
        myQuery = myQuery + 'WHERE P.producto_nuevo = TRUE AND V.variacion_predeterminada = TRUE ';
        ...
        if (rows.length == 0) return response.status(404).json({
            message: "No hay Productos Nuevos"
        });
